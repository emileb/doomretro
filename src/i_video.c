...
void GetScreenResolution(void)
{
    ...
    else
    {
        ...
        if (sscanf(vid_screenresolution, "%10dx%10d", &width, &height) != 2
...
static void I_GetScreenDimensions(void)
{
    ...
    else if (vid_widescreen || keepwidescreenduringanim)
    {
        ...
        WIDEFOVDELTA = atan(dest_rect.w / (dest_rect.h / tan(r_fov * 0.82 * M_PI / 360.0))) * 360.0 / M_PI - r_fov;

        if (WIDEFOVDELTA > MAXWIDEFOVDELTA)
            WIDEFOVDELTA = MAXWIDEFOVDELTA;
